...
import 'package:gromartstore/ui/chat/PlayerWidget.dart';
import 'package:gromartstore/ui/fullScreenImageViewer/FullScreenImageViewer.dart';
import 'package:gromartstore/ui/fullScreenVideoViewer/FullScreenVideoViewer.dart';
...
class _ChatScreenState extends State<ChatScreen> {
                        ...
                        if (snapshot.connectionState == ConnectionState.waiting) {
                          ...
                          if (snapshot.hasData && (snapshot.data?.message.isEmpty ?? true)) {
                            return Padding(
                              ...
                              child: Center(child: showEmptyState('noMessagesYet'.tr(), 'sendNewMessageItWillShowUpHere'.tr())),
                            );
                          } else {
  ...
  Widget buildSubTitle(User friend) {
    String text = friend.active ? 'activeNow'.tr() : 'lastSeenOn'.tr(args: ['${setLastSeen(friend.lastOnlineTimestamp.seconds)}']);
            ...
            children: <Widget>[
              ...
              Positioned.directional(
                  ...
                  child: Container(
                    ...
                    decoration: BoxDecoration(
                        color:
                            homeConversationModel.members.firstWhere((element) => element.userID == messageData.senderID).active ? Colors.green : Colors.grey,
  ...
  Future<bool> _checkChannelNullability(ConversationModel? conversationModel) async {
    if (conversationModel != null) {
      ...
      ConversationModel conversation = ConversationModel(
          creatorId: user.userID, id: channelID, lastMessageDate: Timestamp.now(), lastMessage: 'sentMessage'.tr(args: ['${user.fullName()}']));
  ...
  _sendMessage(String content, Url url, String videoThumbnail) async {
    ...
    if (url.mime.contains('image')) {
      message.content = 'sentImage'.tr(args: ['${MyAppState.currentUser!.firstName}']);
    } else if (url.mime.contains('video')) {
      message.content = 'sentVideo'.tr(args: ['${MyAppState.currentUser!.firstName}']);
    } else if (url.mime.contains('audio')) {
      message.content = 'sentVoiceMessage'.tr(args: ['${MyAppState.currentUser!.firstName}']);
    }
      ...
      actions: <Widget>[
        CupertinoActionSheetAction(
          ...
          onPressed: () async {
            ...
            bool isSuccessful = await _fireStoreUtils.blockUser(homeConversationModel.members.first, 'block'.tr());
            hideProgress();
            if (isSuccessful) {
              Navigator.pop(context);
              _showAlertDialog(
