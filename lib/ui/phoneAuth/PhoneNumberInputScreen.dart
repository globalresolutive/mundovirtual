                ...
                Visibility(
                  ...
                  child: Padding(
                    ...
                    child: Container(
                      ...
                      decoration:
                          BoxDecoration(borderRadius: BorderRadius.circular(25), shape: BoxShape.rectangle, border: Border.all(color: Colors.grey.shade200)),
      ...
      if (_verificationID != null) {
        ...
        if (result != null && result is User && result.role == USER_ROLE_VENDOR) {
          ...
          if (MyAppState.currentUser!.active == true) {
            ...
            showAlertDialog(context, 'accountDisabledContactToAdmin'.tr(), "", true);
          }
        } else if (result != null && result is String) {
          showAlertDialog(context, 'failed'.tr(), result, true);
        } else {
          showAlertDialog(context, 'failed'.tr(), 'notCreateUserWithPhoneNumber'.tr(), true);
        }
      } else {
        await hideProgress();
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
          content: Text('notGetVerificationId'.tr()),
          duration: Duration(seconds: 6),
        ));
      }
    } on auth.FirebaseAuthException catch (exception) {
      hideProgress();
      String message = 'errorOccuredTryAgain';
      switch (exception.code) {
        case 'invalid-verification-code':
          message = 'invalidCodeOrExpired';
          break;
        case 'user-disabled':
          message = 'userDisabled';
          break;
        default:
          message = 'errorOccuredTryAgain';
          break;
      }
      ...
      (String verificationId) {
        if (mounted) {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text(
                'verificationTimeoutRequestNewCode'.tr(),
              ),
            ),
          );
          setState(() {
